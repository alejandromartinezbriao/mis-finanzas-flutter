...
class AiHistoryPage extends StatelessWidget {
        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              // LIMPIEZA DE TÍTULO: Usamos el monthId guardado para evitar IDs técnicos largos
              final String reportId = r['id'].toString();
              final bool isPlanning = reportId.startsWith('plan_');
              final String rawMonthId = r['monthId'] ?? reportId;
              final String monthLabel = rawMonthId.replaceAll('plan_', '').replaceAll('_', ' ').toUpperCase();
                    ...
                    onPressed: () async {
                      final bool? confirm = await showDialog<bool>(
                        ...
                        builder: (ctx) => AlertDialog(
                          ...
                          content: const Text('¿Estás seguro de que quieres eliminar este informe?'),
                          actions: [
                      ...
                      if (confirm == true) await service.deleteAiReport(reportId);
                    },
                  ),
                  children: [
                    Padding(
  ...
  Widget _buildPlanningLeading(String viabilidad) {
    ...
    return CircleAvatar(backgroundColor: color.withOpacity(0.1), child: Icon(Icons.psychology_outlined, color: color, size: 20));
  }
  ...
  Widget _buildPlanningDetails(BuildContext context, Map<String, dynamic> data) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (data['recomendaciones'] != null) ...[
        const SizedBox(height: 12),
        const Text('RECOMENDACIONES', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.indigo, letterSpacing: 0.5)),
        const SizedBox(height: 4),
        ...(data['recomendaciones'] as List).map((r) => Padding(padding: const EdgeInsets.only(bottom: 4), child: Text('• ${r.toString()}', style: const TextStyle(fontSize: 13)))),
      ],
    ]);
  }
